...
public class Penyihir implements Kelas {
    ...
    public void serang(Karakter self, Karakter target) {
        if (self.getMana() >= 20) {
            System.out.println(self.getName() + ", merapal " + sihir + " kepada " + target.getName());
            self.reduceMana(20);
            target.terimaSerangan(30);
        } else {
            System.out.println(self.getName() + ", memukul " + target.getName() + " dengan " + tongkat);
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        ...
        int manaYangDiserap = Math.min(40, target.getMana());
        target.reduceMana(manaYangDiserap);
        self.addMana(manaYangDiserap);
        
        System.out.println(self.getName() + " menyerap " + manaYangDiserap + " mana dari " + target.getName());
