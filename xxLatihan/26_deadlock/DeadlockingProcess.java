...
public class DeadlockingProcess implements Runnable {
    ...
    public void run() {
        ...
        // Tukar peng-alamatan
        if (firstResource.name.compareTo(secondResource.name) > 0){
            firstLock = firstResource;
            secondLock = secondResource;
        } else {
            firstLock = secondResource;
            secondLock = firstResource;
        }
        System.out.println(threadName + ": Mencoba mengunci " + firstLock.name);
        synchronized (firstLock) {
            System.out.println(threadName + ": Berhasil mengunci " + firstLock.name);
            
            // Memberi kesempatan thread lain untuk berjalan dan mengunci resource kedua
            try { Thread.sleep(100); } catch (InterruptedException e) {}

            System.out.println(threadName + ": Mencoba mengunci " + secondLock.name);
            synchronized (secondLock) {
                System.out.println(threadName + ": Berhasil mengunci " + secondLock.name);
                
                // Melakukan pekerjaan
                System.out.println(threadName + ": Bekerja dengan kedua resource...");
            }
            System.out.println(threadName + ": Melepas kunci " + secondLock.name);
        }
        System.out.println(threadName + ": Melepas kunci " + firstLock.name + " dan selesai.");
    }
}
